...
export default function LootPanel({ controller, actions, eventBus, session, initialPileId, onClose }) {
  useTick(eventBus)
  ...
  const loot = (item, qty = null) => {
    if (!target) return
    actions?.transferItem({ itemId: item.id, toActorId: target.id, quantity: qty })
  }
  ...
  const newPile = () => actions?.createLootPile('Loot')
  const deletePile = () => {
