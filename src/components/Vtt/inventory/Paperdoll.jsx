...
function Slot({ slot, item, locked }) {
  return (
    <Droppable id={`slot:${slot}`} data={{ slot }} disabled={locked}>
...
export default function Paperdoll({ selected, equipment, locked }) {
  ...
  return (
    <div className="inv-paperdoll">
      <div className="inv-slotcol">
        {LEFT.map(s => <Slot key={s} slot={s} item={equipment[s]} locked={locked} />)}
      </div>
      <div className="inv-hero">
        {hero ? <img src={hero} alt={selected?.name} /> : <div className="inv-hero-fallback">{initial}</div>}
      </div>
      <div className="inv-slotcol">
        {RIGHT.map(s => <Slot key={s} slot={s} item={equipment[s]} locked={locked} />)}
      </div>
    </div>
  )
}
