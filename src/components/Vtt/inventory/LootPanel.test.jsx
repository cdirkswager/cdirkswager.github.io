...
describe('LootPanel', () => {
  ...
  it('DM clicking New pile calls createLootPile', () => {
    const ctrl = makeController()
    const actions = { createLootPile: vi.fn(), transferItem: vi.fn() }
    render(<LootPanel controller={ctrl} actions={actions} eventBus={{ on: () => () => {} }} session={{ userId: 'dm-1', role: 'dm' }} />)
    fireEvent.click(screen.getByText('+ New pile'))
    expect(actions.createLootPile).toHaveBeenCalled()
  })
  ...
  it('Player clicking Loot calls transferItem', () => {
    ...
    ctrl.itemMap.set(item.id, item)
    const actions = { createLootPile: vi.fn(), transferItem: vi.fn() }
    render(<LootPanel controller={ctrl} actions={actions} eventBus={{ on: () => () => {} }} session={{ userId: 'u1', role: 'player' }} />)
    fireEvent.click(screen.getByRole('button', { name: 'Loot' }))
    expect(actions.transferItem).toHaveBeenCalledWith({ itemId: 'item-4', toActorId: char.id, quantity: null })
  })
})
